        ...
        if choice=='Modeling':
            ...
            if file_csv:
                ...
                if 'user_id' not in data.columns:
                    ...
                    df_final, df_filtered_users = xu_ly_du_lieu(data)
                    ...
                    df_final,centers_df_minmax_centroids,scaler,kmeanModel = chuan_doan_du_lieu(df_final)

                    #Vẽ biểu đồ
                    st.subheader('4. Model deployment and demonstration')
                    ve_bieu_do(df_final,centers_df_minmax_centroids)
            ...
            Name, Age, Volume, Device, Created_days,Login,Vungkinhte, Bank = nhap_thong_tin()
            df_final,centers_df_minmax_centroids,scaler,kmeanModel = all_to_model(df_pre)


            if Volume > 0:
                if st.button('Prediction'):
                    thong_bao(df_final,centers_df_minmax_centroids,scaler,kmeanModel, Name, Age, Volume, Device, Created_days,Login,Vungkinhte,Bank)
                else:
            ...
            if file_csv:
                ...
                if 'user_id' not in data.columns:
                    ...
                    #Tải dữ liệu xuống
                    st.write('*'*40)
                    download_csv(df_final[['user_id','label']])
